...
const AUTHOR_URL = 'https://books.apple.com/us/author/lucky-gupta/id1525829357';
        ...
        <section id="books" className="mt-32 flex w-full justify-center">
            <div className="w-full max-w-6xl px-4">
                ...
                <div className="grid grid-cols-2 gap-x-6 gap-y-10 sm:grid-cols-3 md:grid-cols-5">
                    {books.map((book) => (
                        <Link key={book.title} href={book.appleUrl} target="_blank" className="group">
                            <div className="apple-book transition group-hover:shadow-2xl">
                                {/* Background print */}
                                <Image src={book.image} alt="" fill className="apple-book-bg" aria-hidden />

                                {/* Foreground print */}
                                <Image src={book.image} alt={book.title} fill className="apple-book-fg" sizes="(max-width: 768px) 50vw, 20vw" />
                            </div>

                            <div className="mt-3">
                                <p className="text-sm font-semibold leading-snug text-black dark:text-white">{book.title}</p>
                                <p className="text-xs text-neutral-500 dark:text-neutral-400">{book.year}</p>
                            </div>
                        </Link>
                    ))}
                </div>
