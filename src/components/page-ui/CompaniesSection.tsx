        ...
        <section id="companies" className="mt-32 w-full">
            <div className="mx-auto w-full max-w-6xl px-4">
                ...
                <div className="mb-10 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <h2 className="text-2xl font-bold text-black dark:text-white sm:text-3xl">List of Lucky Gupta Group of companies</h2>
                ...
                {/* Responsive Grid */}
                <div
                    className="grid grid-cols-2 justify-center gap-x-6
                        gap-y-10
                        sm:grid-cols-3
                        md:grid-cols-4
                        lg:grid-cols-5"
                >
                    {companies.map((company) => (
                        <Link key={company.id} href={COMPANY_SITE_URL} target="_blank" className="group flex flex-col items-center text-center">
                            {/* Logo Box */}
                            <div
                                className="flex size-[140px] items-center
                              justify-center rounded-lg
                              bg-white shadow-sm
                              transition group-hover:shadow-md sm:size-[160px] md:size-[180px]"
                            >
                                <Image src={company.image} alt={company.name} width={180} height={180} className="object-contain" />
                            </div>

                            {/* Name */}
                            <p className="mt-3 max-w-[160px] text-sm font-semibold leading-snug text-neutral-800 dark:text-white">{company.name}</p>
                        </Link>
                    ))}
                </div>
            </div>
